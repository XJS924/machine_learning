...
class Pandas_Util:
    ...
    def read_file(self,filename,year,month):
        """
        读取Excel文件
        ...
        """
        try:
            year_month = '{}年{:02}月'.format(year,month)
            file = [x for x in self.files if filename in x and  year_month in x ][0]
...
def gengerate_note():
    configPa= ConfigPa()
    this_year = int(configPa.get_option('current', 'year'))
    this_month = int(configPa.get_option('current', 'month'))
    ...
    df_mid_vol_fault_this=pandas_util.read_file("配网故障",this_year,this_month)
    # 2017 中压故障工单
    df_mid_vol_fault_last = pandas_util.read_file("配网故障",last_year,this_month)
    ...
    df_low_vol_fault_current = pandas_util.read_file('报修查询', this_year, this_month)
    df_low_vol_fault_current = pandas_util.dataframe_setindex(df_low_vol_fault_current, '接单登记时间')

    df_low_vol_fault_last = pandas_util.read_file('报修查询', last_year, this_month)
    ...
    df_export_low_vol = pandas_util.read_file("公变出口", this_year, this_month)
    ...
    for k, v in df_export_low_vol[this_date].groupby(['单位']).groups.items():
        ...
        d_4_1_3[k].append('{:.2f}%'.format(len(v) / float(configPa.get_option('公变出口低电压数',k))))
        s += len(v)
    ...
    for k, v in df_class_d_user[this_date].groupby(['单位']).groups.items():
        ...
        d_4_1_4[k].append('{:.2f}%'.format(len(v) / float(configPa.get_option('公变出口低电压数',k))))
        s += len(v)
    ...
    df_load_change = pandas_util.read_file("配变负载", this_year, this_month)
    ...
    df_change_stop = pandas_util.read_file("公变停运", this_year, this_month)
    ...
    d_stop['合计'].append(round(sum(ddd_stop.values()) / int(configPa.get_option('线路总数', '合计')) * 100,2))
    d_stop['合计'].append(round(sum(ddd_stop.values()) / int(configPa.get_option('线路总数', '合计')) * 100, 2))
    ...
    repate_rate= round(sum([sum(v.values()) for v in line_dict.values()]) * 100 / int(configPa.get_option('线路总数', '合计')), 2)
