...
this_year = int(configParser.get('current','year'))
this_month = int(configParser.get('current','month'))
last_year = this_year - 1
last_month = this_month - 1
this_date = '%d-%02d' % (this_year, this_month)
round_date = '%d-%02d' % (this_year, last_month)
last_date = '%d-%02d' % (last_year, this_month)
last_date_last = '%d-%02d' % (last_year, last_month)
unit = {'六合营业部': '六合', '江宁营业部': '江宁', '浦口营业部': '浦口', '溧水县公司': '溧水', '配电运检室': '城区', '高淳县公司': '高淳'}
path = 'G:/notebook/201810/gjdw/设备监测数据包/'

files = os.listdir(path)

# 2018 中压故障工单
mid_vol_file_this = [x for x in files if str(this_year) in x and "配网故障" in x][0]
df_mid_vol_fault_this = pd.read_excel(path + mid_vol_file_this)
# 2017 中压故障工单
mid_vol_file_last = [x for x in files if str(last_year) in x and "配网故障" in x][0]
# mid_vol_file_last
df_mid_vol_fault_last = pd.read_excel(path + mid_vol_file_last)
df_mid_vol_fault_this = df_mid_vol_fault_this.set_index('故障时间')
df_mid_vol_fault_last = df_mid_vol_fault_last.set_index('故障时间')
df_mid_vol_fault_current_month = df_mid_vol_fault_this[this_date]  # 本期
df_mid_vol_fault_round_month = df_mid_vol_fault_this[round_date]  # 上期
df_mid_vol_fault_last_month = df_mid_vol_fault_last[last_date]  # 去年同期

s = '报修查询'
fault_order_file = [x for x in files if str(this_year) in x and s in x][0]
df_low_vol_fault_current = pd.read_excel(path + fault_order_file)
df_low_vol_fault_current.接单登记时间 = pd.to_datetime(df_low_vol_fault_current.接单登记时间)
df_low_vol_fault_current = df_low_vol_fault_current.set_index('接单登记时间')

fault_order_file_last = [x for x in files if str(last_year) in x and str(this_month) in x and s in x][0]
df_low_vol_fault_last = pd.read_excel(path + fault_order_file_last)
df_low_vol_fault_last.接单时间 = pd.to_datetime(df_low_vol_fault_last.接单时间)
df_low_vol_fault_last = df_low_vol_fault_last.set_index('接单时间')
...
def gengerate_note():
    ...
    plt.pie(x=dd_fault_equ.values(), autopct='%2.0f%%',shadow=False, labeldistance=5, startangle=90, pctdistance=1.1)
    plt.legend(dd_fault_equ.keys(), loc='lower right', bbox_to_anchor=(1.35, 0.3))
    plt.title('全市', fontsize=20)
    fig = plt.gcf()
    fig.savefig("all_equ_fault.png", dpi=300)
    plt.figure(figsize=(10, 10))
    for u in ['城区', '江宁', '六合', '浦口', '溧水', '高淳']:
        plt.subplot(p_row)
        fracs = d_fault_equ[u][0]
        labels = d_fault_equ[u][1]
        plt.pie(x=fracs, autopct='%2.0f%%',shadow=False, labeldistance=5, startangle=90, pctdistance=1.15)
        plt.legend(labels, loc='lower right', bbox_to_anchor=(1.6, 0.3))
        plt.title(u, fontsize=20, loc='center')
        p_row += 1
    fig = plt.gcf()
    fig.savefig("each_equ_fault.png", dpi=300)
    plt.close('all')
    ...
    plt.pie(x=cause_value, autopct='%2.0f%%',shadow=False, labeldistance=5, startangle=90, pctdistance=1.1)
    ...
    ##（4）低压故障数量统计
    ####  2018 加入单位
    df_low_vol_fault_current['单位'] = ''
    team = []
    for index, row in df_low_vol_fault_current.iterrows():
        #     row['部门']
        unit0 = ''
        if type(row['抢修队伍/班组']) == str:
            if "溧水" in row['抢修队伍/班组']:
                unit0 = "溧水"
            elif "高淳" in row['抢修队伍/班组']:
                unit0 = "高淳"
            elif '六合' in row['抢修队伍/班组']:
                unit0 = "六合"
            elif "浦口" in row['抢修队伍/班组']:
                unit0 = "浦口"
            elif "江宁" in row['抢修队伍/班组']:
                unit0 = "江宁"
            else:
                unit0 = "城区"
        row.单位 = unit0
        team.append(unit0)
    df_low_vol_fault_current['单位'] = team
    ##3 2017 加入单位
    df_low_vol_fault_last['单位'] = ''
    team_last = []
    for index, row in df_low_vol_fault_last.iterrows():
        unit0 = ''
        if type(row['供电单位']) == str:
            if "溧水" in row['供电单位']:
                unit0 = "溧水"
            elif "高淳" in row['供电单位']:
                unit0 = "高淳"
                ...
                unit0 = "城区"
        row.单位 = unit0
        team_last.append(unit0)
    df_low_vol_fault_last['单位'] = team_last
    ### 低压 本期
    df_low_vol_fault_current_month = df_low_vol_fault_current[this_date]
    ### 低压上期
    df_low_vol_fault_round_month = df_low_vol_fault_current[round_date]
    #### 低压 去年同期
    df_low_vol_fault_last_month = df_low_vol_fault_last[last_date]
    ...
    if len(dd.keys())>9:
        ...
        dd_vs=dd.values()
        dd_key = dd.keys()
    plt.pie(x=dd_vs, autopct='%2.0f%%', shadow=False, labeldistance=5, startangle=90, pctdistance=1.1)
    plt.legend(dd_key, loc='lower right', bbox_to_anchor=(1.5, 0.3))
    plt.title('全市', fontsize=20)
    plt.show()
    fig = plt.gcf()
    ...
    for u in ['城区', '江宁', '六合', '浦口', '溧水', '高淳']:
        ...
        plt.legend(labels, loc='lower right', bbox_to_anchor=(1.75, 0.05))
        plt.title(u, fontsize=20, loc='center')
        p_row += 1
    ...
    export_low_vol_file = [x for x in files if '公变出口' in x][0]
    df_export_low_vol = pd.read_excel(path + export_low_vol_file)
    df_export_low_vol.columns = df_export_low_vol.loc[0].values
    df_export_low_vol = df_export_low_vol[1:]
    df_export_low_vol.发生时刻 = pd.to_datetime(df_export_low_vol.发生时刻)
    df_export_low_vol = df_export_low_vol.set_index('发生时刻')
    #  设置相应单位
    df_export_low_vol['单位'] = ''
    for index, row in df_export_low_vol.iterrows():
        #     row['部门']
        if "溧水" in row['部门']:
            row['单位'] = "溧水"
        elif "高淳" in row['部门']:
    ...
    for k, v in df_export_low_vol[this_date].groupby(['单位']).groups.items():
        ...
        d_4_1_3[k].append('{:.2f}%'.format(len(v) / float(configParser.get('公变出口低电压数',k))))
        s += len(v)
    d_4_1_3_s = sorted(d_4_1_3.items(), key=lambda a: a[1], reverse=True)
    s_4_1_3 = "本期公变出口低电压共{}台，低电压比例{:.1f}%，其中公变低电压户数最多的为{}{}台。".format(s, s / 1000, d_4_1_3_s[0][0], d_4_1_3_s[0][1][0])
    d_4_1_3['合计'].append(s)
    ...
    ######################
    #########4）D类用户出口低电压############
    class_d_user_file = [x for x in files if 'D类用户' in x][0]
    df_class_d_user = pd.read_excel(path + class_d_user_file)
    df_class_d_user.columns = df_class_d_user.loc[0].values
    df_class_d_user = df_class_d_user[1:]
    df_class_d_user.发生日期 = pd.to_datetime(df_class_d_user.发生日期)
    df_class_d_user = df_class_d_user.set_index('发生日期')
    #  设置相应单位
    df_class_d_user['单位'] = ''
    for index, row in df_class_d_user.iterrows():
        #     row['部门']
        if "溧水" in row['部门']:
            row['单位'] = "溧水"
        elif "高淳" in row['部门']:
    ...
    for k, v in df_class_d_user[this_date].groupby(['单位']).groups.items():
        ...
        d_4_1_4[k].append('{:.2f}%'.format(len(v) / float(configParser.get('公变出口低电压数',k))))
        s += len(v)
    ...
    load_change_file = [x for x in files if '配变负载' in x ][0]#and str(this_month) in x][0]
    df_load_change = pd.read_excel(path + load_change_file)
    df_load_change.columns = df_load_change.loc[0].values
    df_load_change = df_load_change[1:]
    df_load_change.发生时刻 = pd.to_datetime(df_load_change.发生时刻)
    df_load_change = df_load_change.set_index('发生时刻')
    #  设置相应单位
    df_load_change['单位'] = ''
    for index, row in df_load_change.iterrows():
        #     row['部门']
        if "溧水" in row['部门']:
            row['单位'] = "溧水"
        elif "高淳" in row['部门']:
    ...
    # table.rows[1].cells[2].add_paragraph('与上期重复数')
    for row, obj_row in enumerate(table.rows[2:]):
        for col, cell in enumerate(obj_row.cells[0:1]):
            cell.text = unit_sort[row]
    for row, obj_row in enumerate(table.rows[2:]):
    ...
    change_stop_file = [x for x in files if '公变停运' in x][0]
    # change_stop_file
    df_change_stop = pd.read_excel(path + change_stop_file)
    df_change_stop.columns = df_change_stop.loc[0].values
    df_change_stop = df_change_stop[1:]
    df_change_stop.失电时间 = pd.to_datetime(df_change_stop.失电时间)
    df_change_stop = df_change_stop.set_index('失电时间')
    d_stop = {}
    dd_stop = {}
    ddd_stop = {}
    line_count = {}
    for k in ['城区', '江宁', '六合', '浦口', '溧水', '高淳']:
        d_stop[k] = []
        line_count[k] = int(configParser.get('线路总数', k))
        dd_stop[k] = 0
        ddd_stop[k] = 0
    #  设置相应单位
    df_change_stop['单位'] = ''
    for index, row in df_change_stop.iterrows():
        #     row['部门']
        if "溧水" in row['部门']:
            row['单位'] = "溧水"
        elif "高淳" in row['部门']:
    ...
    d_stop['合计'].append(round(sum(ddd_stop.values()) / int(configParser.get('线路总数', '合计')) * 100,2))
    d_stop['合计'].append(round(sum(ddd_stop.values()) / int(configParser.get('线路总数', '合计')) * 100, 2))

    s_4_3 = "对本月南京地区配变停运情况进行统计，配变停运总次数{}台次，停运2次{}台，停运3次{}台，停运4次{}台。配变重复停运占比最高为{}{}%，最低为{}{}%。".format(sum(ddd_stop.values()), two, three, four, highest, max_stop, lowest, min_stop)
    ...
    for k in ['城区', '江宁', '六合', '浦口', '溧水', '高淳']:
        ...
        rate = round(sum(line_dict[k].values()) / int(configParser.get('线路总数', k)) * 100, 2)
    ...
    repate_rate= round(sum([sum(v.values()) for v in line_dict.values()]) * 100 / int(configParser.get('线路总数', '合计')), 2)
...
###################
    ...
    import time
    name = 'G:/月报/月报_{}.docx'.format(time.strftime("%m_%d_%H_%M", time.localtime(time.time())))
    document.save(name)
